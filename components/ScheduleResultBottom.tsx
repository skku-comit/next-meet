...
const ScheduleResultBottom = React.memo(function ScheduleResultBottom({
  width,
  setShowResult,
  showResult,
  showMember,
  ...
  let max_checked_mem_sche: string[] = Math.max(...checked_mem_num) == 0
      ? []
  ...
  const handleConfirm = async (newSchedule: Date[]) => {
    ...
    newSchedule.forEach((sche: Date) => {
      const time =
        new Date(sche).getHours() + ":" + new Date(sche).getMinutes();

      if (week) {
        (fixedMeeting as WeeklyFixedDate[]).push({
          day: WEEKDAY[new Date(sche).getDay()] as DaysOfWeek,
          timeRange: [time],
        });
      } else {
        (fixedMeeting as FixedDate[]).push({
          date: new Date(sche),
          timeRange: [time],
        });
      }
    ...
    <div className="flex flex-col h-full justify-between z-25 overflow-hidden overflow-x-auto px-5 pt-3 pb-2 bg-[#f8f9fa] rounded ">
      <div className={`h-full flex flex-row`}>
        <div className={`w-2/4 pr-2`}>
          <div
            className={`w-full pb-2 mr-1 grid grid-column gap-2 ${
              width > 768 ? "pl-3" : "border-l-3"
            }`}
          >
            <div className={`pt-2 mr-1`}>
              ...
              <ul
                className={`${scheduleResultCSS.result_scrolling4} px-2 min-h-6 grid grid-column gap-1`}
                style={{
                  maxHeight: isHost
                    ? confirm == 2 || confirm == 3
                      ? height - 60
                  ...
                  && sortedList.map((sche: Date, index) => {
                      ...
                      start_sche.setMinutes(
                        start_sche.getMinutes() -
                          30 * (term != 0 ? term - 1 : term)
                      );
                      end_sche.setMinutes(end_sche.getMinutes() + 30);
                      term = 0;
                      return (
                        <li key={index}
                          className="bg-[lightgray] px-3 pt-3 pb-2 rounded cursor-pointer">
                          <p className="inline-block">
                            {(week
                              ? ""
                              : start_sche.toLocaleDateString("ko-KR")) +
                              "(" +
                              (lang == "ko"
        ...
        <div className={`w-2/4 border-l-2 ${
            width > 768 ? "grid-column" : "grid-row"
          } mr-1 grid gap-2`}
        >
          <div className={`w-full mr-1 grid grid-column gap-2 pl-3`}>
            <div className="pt-2 mr-1">
              ...
              <hr className="border-t-2 my-1 mb-2" />
              <ul className={`${scheduleResultCSS.result_scrolling4} px-2 flex flex-col gap-2`}
                  style={{
                  maxHeight: isHost
                    ? confirm == 2 || confirm == 3
                      ? height - 60
        ...
        <div>
          <IoMdClose
            className={`${scheduleResultCSS.closeBtn} fixed right-0.5 cursor-pointer`}
            onClick={() => {
              setShowResult(false);
            }}
          />
      ...
      {isHost ? (
        select ? (
          ...
          <div className={`sticky bottom-0 flex flex-row gap-2 mt-2 pr-2`}>
            <div
              className={`w-full p-2 pt-3 rounded hover:font-bold ${
                confirm == 1
                  ? "bg-[#ced4da]"
                  : select == 1
                  ? "bg-[#868e96]"
