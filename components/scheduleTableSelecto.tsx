...
const ScheduleTableSelecto = React.memo(function ScheduleTableSelecto({
  ...
  const [dateList, setDateList]: [Date[], Function] = useState(
    eventDateArr
      ? eventDateArr.sort(
  ...
  const [timePeriod, setTimePeriod] = useState(
    eventTimeInfo
      ? { startTime: eventTimeInfo.startTime, endTime: eventTimeInfo.endTime }
      : dummyTimePeriod
  );
  const startTimeHour: number =
    parseInt(timePeriod.startTime.split(":")[0]) +
    parseFloat(timePeriod.startTime.split(":")[1] == "30" ? "0.5" : "0");
  const endTimeHour: number =
    timePeriod.endTime == "00:00"
      ? 24
      : parseInt(timePeriod.endTime.split(":")[0]) +
        parseFloat(timePeriod.endTime.split(":")[1] == "30" ? "0.5" : "0");
  ...
  if (eventParti) {
    for (let i = 0; i < eventParti.length; i++) {
      ...
      givenEventScheList.checked_num[time_str] =
        prevTotalMem == 0 ? 0 : eventParti[i].user.length / prevTotalMem;
      givenEventScheList.member[time_str] = [];
      for (let j = 0; j < eventParti[i].user.length; j++) {
        givenEventScheList.member[time_str].push(
          eventParti[i].user[j]?.userName
        );
      }
    }
  }
  ...
  useEffect(() => {
    ...
    for (const key in scheduleList.checked_num) {
      revisedScheduleList.checked_num[key] =
        totalMem == 0
          ? 0
          : (scheduleList.checked_num[key] * prevTotalMem) / totalMem;
    }
          ...
          (prevSche: {
            ...
            checked_num: {
              ...
              [sche_str]:
                totalMem == 0
                  ? 0
                  : (prevSche.checked_num[sche_str] * totalMem - 1) / totalMem,
            },
            member: {
              ...prevSche.member,
              [sche_str]: new_member,
          ...
          setScheduleList(
            (prevSche: {
              ...
              console.log(
                ...
                totalMem,
                prevTotalMem,
                prevSche.checked_num[sche_str],
                totalMem == 0 ? 1 : totalMem
              );
              return {
                checked_num: {
                  ...prevSche.checked_num,
                  [sche_str]:
                    totalMem == 0
                      ? 0
                      : (prevSche.checked_num[sche_str] * totalMem + 1) /
                        totalMem,
                },
                member: {
                  ...prevSche.member,
  ...
  return (
    <div
      className="w-full overflow-hidden overflow-x-auto p-5 bg-[#f8f9fa] rounded"
      hidden={state == "EDIT" && confirm == 1 ? true : false}
    >
      <div
        className={`flex flex-row gap-2 justify-between font-bold items-center cursor-pointer w-full ${
          width > 768 || open ? "pb-2" : ""
        }`}
        onClick={() => {
          setOpen((prevOP: boolean) => !prevOP);
        }}
      >
        <p>
          {state == "EDIT"
            ? lang == "ko"
              ? "내 일정을 표시 및 수정하십시오."
              : "Check or Edit Available Schedule"
            : state == "CONFIRM"
            ? lang == "ko"
              ? "확정할 일정을 표시하십시오."
            ...
            : lang == "ko"
            ? "전체 일정"
            : "Total Schedule Table"}
        </p>
        {width > 768 ? "" : !open ? <FaAngleUp /> : <FaAngleDown />}
      </div>
      {width > 768 || open ? (
        <div className={`w-full animate-[smoothAppear_1s]  ${scheduleTableCSS.table_spacing} border-separate table-scrolling pt-3 border-t-2`}>
          <ScheduleSelector
            selection={
              state == "EDIT"
                ? schedule.schedule
            ...
            renderDateLabel={(date: Date) => {
              if (!week) {
                ...
                return (
                  <div className="w -full min-w-[70px] h-[25px]">
                    {new Date(dateList[index]).getMonth() +
                      1 +
                      "/" +
                      new Date(dateList[index]).getDate()}
                    {"("}
                    {lang == "ko"
                      ? WEEKDAY[new Date(dateList[index]).getDay()]
                      : WEEKDAY2[new Date(dateList[index]).getDay()]}
                    {")"}
                  </div>
                );
              }
              return (
                <div className={`${ width > 600 ? "w-full" : scheduleTableCSS.date_label} h-full`}
                  style={{ height: "25px", minWidth: "70px" }}>
                  {lang == "ko"
                    ? DayList[
                        date.getDay() - week_startDate.getDay() < 0
                          ? DayList.length - 1
                          : date.getDay() - week_startDate.getDay()
                      ]
                    : sortedSelectedWeekDay[
                        date.getDay() - week_startDate.getDay() < 0
                          ? sortedSelectedWeekDay.length - 1
                          : date.getDay() - week_startDate.getDay()
                      ]}
                </div>
              );
            ...
            renderTimeLabel={(time) => {
              return (
                <div className={`sticky top-0 left-0 bg-[#f8f9fa] pr-1 z-20`}>
                  {time.getHours() < 10
                    ? "0" + time.getHours()
                    : time.getHours()}
            ...
            renderDateCell={
              state == "EDIT" || state == "CONFIRM"
                ...
                : (datetime, selected, refSetter) => {
                    ...
                    return (
                      <div className={`relative w-full h-full ${scheduleTableCSS.date_cell}`}
                        style={{
                        ...
                        {(width > 768 && confirm == 2) ||
                        scheduleList.member[datetimeStr]?.length >0 &&
                          <div className={`${scheduleTableCSS.date_cell_popup} z-10`}>
                            {scheduleList.member[datetimeStr] &&
                              <ul>
                                {scheduleList.member[datetimeStr].map((member, idx) => 
